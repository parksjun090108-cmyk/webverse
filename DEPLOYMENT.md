...
- Build: 의존성 설치, PostgreSQL Prisma Client 생성, 서버 컴파일, DB 마이그레이션과 공식 사이트 seed
- Start: 컴파일된 Express API만 즉시 실행
- Health check: `/api/ready`

무료 Render 인스턴스는 수면 해제 때마다 Start command를 다시 실행합니다. 따라서 마이그레이션과 seed를 Start command에 넣지 않습니다. 그렇지 않으면 서버가 포트를 열기 전에 DB 준비 작업부터 반복해 콜드 스타트가 크게 느려집니다.
...
그 후 `extension` 폴더를 ZIP으로 묶어 Chrome Web Store에 등록합니다. 확장 프로그램은 `history`, `bookmarks` 권한을 사용하므로 개인정보처리방침에 아래 내용을 명시해야 합니다.

- 계정 연결 전에는 방문 정보를 수집하지 않음
- 첫 우주 생성 전 최근 30일 방문 상위 20개와 북마크 목록을 브라우저 안에서 계산하고 사용자가 선택하도록 함
- 선택한 사이트의 도메인, 북마크 이름, 방문 횟수, 최근 방문일, 북마크 여부만 서버에 전송함
- 전체 URL, 검색어, 방문 페이지 제목은 서버에 전송하지 않음
- 이후 방문은 내 우주에 있는 사이트만 집계하며 브라우저 북마크 변경은 WebVerse 즐겨찾기로만 단방향 반영함
- WebVerse가 브라우저 북마크를 생성하거나 삭제하지 않음
