...
export function RequestDrawer({ token, initialRequest, categories, onClose, onCompleted }: Props) {
  ...
  return <div className="drawer-layer" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <aside className="request-drawer" aria-label="등록 요청 상세">
      ...
      <div className="request-facts"><div><span>서로 다른 발견자</span><strong><Users size={14} /> {request.site._count.discoveries}명</strong></div><div><span>개인 우주 등록</span><strong>{request.site._count.userSites}개</strong></div><div><span>요청 시각</span><strong>{formatDate(request.requestedAt)}</strong></div><div><span>최초 발견자</span><strong>{request.site.createdBy?.nickname ?? '알 수 없음'}</strong></div></div>
